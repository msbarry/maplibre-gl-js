...
export class HeightTile {
    ...
    static fromRawDem(demTile: DEMData): HeightTile {
        return new HeightTile(demTile.dim, demTile.dim, (x, y) => {
            const value = x < -1 || y < -1 || x >= demTile.dim + 1 || y >= demTile.dim + 1 ? NaN : demTile.get(x, y);
